...
with tab3:
    ...
    st.markdown("Live arithmetic breakdown showing every raw data point entered on the dashboard, individual utility weighting, and constraint compliance.")

    with st.expander("1. Raw Data Input Verification Table"):
        st.markdown("This table exposes the live parameters currently configured in your session tables, confirming exact values evaluated by the engine:")
        raw_audit_df = edited_nodes.copy()
        st.dataframe(raw_audit_df, use_container_width=True)

    with st.expander("2. Line-by-Line Objective Utility Calculation"):
        st.markdown("""
        The solver maximizes a multi-objective utility function for each node $n$:
        $$\\text{Utility}_n = (RiskReduction_n \\cdot x_n) + (0.5 \\cdot LeadTimeSaved_n \\cdot x_n)$$
        Below is the step-by-step arithmetic evaluation for every node based on current user selections ($x_n = 1$ if selected, $0$ if rejected):
        """)
        ...
        for n in nodes:
            ...
            total_utility_score += node_utility
            utility_rows.append({
                "Node": n,
                "Status ($x_n$)": "Selected (1)" if x_val == 1 else "Rejected (0)",
                "Risk Component": f"{r_val} × {x_val} = {r_val * x_val}",
                "Lead Time Component": f"0.5 × {lt_val} × {x_val} = {0.5 * lt_val * x_val}",
                "Total Node Utility": f"{node_utility:.2f}"
            })
        st.dataframe(pd.DataFrame(utility_rows), use_container_width=True)
        st.info(f"**Cumulative Portfolio Utility Score:** {total_utility_score:.2f}")

    with st.expander("3. Financial Capital Ledger & Constraint Validation"):
        st.markdown("""
        **Capital Verification Equation:**
        $$\\text{Net Capital Deployed} = \\sum (Cost_n \\cdot x_n) - \\sum (BundleDiscount_b \\cdot b)$$
        """)
        
        ledger_rows = []
        for n in selected_nodes:
            ledger_rows.append({
                "Item Type": "Intervention Node",
                "Description": n,
                "Amount ($)": costs[n]
            })
        for b_name in active_bundle_names:
            b_disc = bundle_vars[b_name]["discount"]
            ledger_rows.append({
                "Item Type": "Synergy Discount",
                "Description": f"Bundle: {b_name}",
                "Amount ($)": -b_disc
            })
        ...
        st.markdown(f"""
        * **Gross Costs Sum:** ${base_cost_spent:,.2f}
        * **Total Discounts Applied:** -${applied_bundle_discounts:,.2f}
        * **Final Net Spend:** **${final_cost_spent:,.2f}**
        * **Active Budget Limit:** ${cap_limit:,.2f}
        * **Constraint Check (${final_cost_spent:,.2f} \\le ${cap_limit:,.2f}):** **PASSED (Valid)**
        """)

    with st.expander("4. Risk Mitigation & Endpoint Proof"):
        st.markdown(f"""
        * **Starting Baseline Risk:** {baseline_risk:.2f}%
        * **Sum of Active Risk Reductions:** {sum(risks[n] for n in selected_nodes):.2f}%
        * **Floor Check (Min Risk = 0%):** Risk cannot drop below zero.
        * **Final Calculated Optimized Risk:** 
          $$\\max(0, {baseline_risk} - {sum(risks[n] for n in selected_nodes)}) = \\mathbf{{{optimized_risk:.2f}\\%}}$$
        """)
