...
with tab3:
    ...
    with st.expander("📌 View Complete System Master Equation (Zero Omissions)", expanded=True):
        st.markdown("### **I. Hybrid Objective Function Selection (Mode Dependent)**")
        ...
        st.markdown("**Commercial Mode: Portfolio Utility Maximization Function**")
        st.latex(r"\mathcal{U}_{\text{system}} = \sum_{n \in N} \left[ w \cdot R_n^{0.85} + (1-w) \cdot L_n \right] x_n")
        ...
        st.markdown("5. **Target Risk Threshold Lower Bound:**")
        st.latex(r"\sum_{n \in N} (R_n^{0.85} x_n) \ge \max\left(0, \text{Risk}_{\text{baseline}} - \text{Risk}_{\text{target}}\right)")
        ...
        st.markdown("4. **Stochastic Outcome Distribution:**")
        st.latex(r"\text{Risk}_{\text{sim}}^{(i)} = \max\left(0, \text{Risk}_{\text{baseline}} - \sum_{n \in N} R_n^{0.85} x_n \max\left(0.4, 1.0 + 0.12 \, Z_{\text{corr}, n}^{(i)}\right)\right)")
