...
with tab3:
    st.subheader("🔍 Formal Mathematical Audit & Traceability Trail")
    st.markdown("Complete computational proof and constraint verification for executive review and corporate compliance.")

    with st.expander("1. Objective Function & Mathematical Formulation"):
        if enable_target_mode:
            st.markdown("""
            * **Optimization Type:** Minimization / Target Goal Satisfaction (`pl.LpMaximize` objective proxy)
            * **Objective Formulation:** Minimize Total Net Capital Deployment $\sum (Cost_n \cdot x_n) - \sum (Discount_b \cdot b)$
            * **Primary Target Constraint:** System Risk Reduction $\sum (RiskReduction_n \cdot x_n) \ge Baseline - Target Goal$
            """)
        else:
            st.markdown("""
            * **Optimization Type:** Multi-Objective Mixed-Integer Linear Program (MILP) (`pl.LpMaximize`)
            * **Objective Formulation:** Maximize Value Score = $\sum \left( Risk_n \cdot x_n + 0.5 \cdot LeadTime_n \cdot x_n \right)$
            * **Explanation:** The solver weighs risk reduction as primary utility, augmented by a weighted secondary factor for lead time improvements.
            """)

    with st.expander("2. Binary Decision Vector Matrix ($x_n$ Solved State)"):
        decision_audit = []
        for n in nodes:
            val = pl.value(x[n])
            decision_audit.append({
                "Node Name": n,
                "Action": actions.get(n, "N/A"),
                "Binary Decision ($x_n$)": int(val) if val is not None else 0,
                "Unit Cost ($)": costs[n],
                "Risk Reduction (%)": risks[n],
                "Lead Time Saved (Days)": lead_times[n]
            })
        st.dataframe(pd.DataFrame(decision_audit), use_container_width=True)

    with st.expander("3. Capital Constraint & Budget Headroom Calculation"):
        raw_spend = sum(costs[n] for n in selected_nodes)
        discount_val = applied_bundle_discounts
        net_spend = final_cost_spent
        cap_limit = 1000000000 if enable_target_mode else total_budget
        headroom = cap_limit - net_spend

        col_a, col_b, col_c = st.columns(3)
        col_a.metric("Gross Node Cost Sum", f"${raw_spend:,.2f}")
        col_b.metric("Total Bundle Discounts", f"-${discount_val:,.2f}")
        col_c.metric("Net Capital Deployed", f"${net_spend:,.2f}")

        st.markdown(f"""
        * **Constraint Equation Checked:** $Net Spend \\le Budget Cap$
        * **Evaluation:** ${net_spend:,.2f} \\le ${cap_limit:,.2f} $\rightarrow$ **VALID** (Constraint Satisfied)
        * **Remaining Capital Headroom:** ${headroom:,.2f}
        """)

    with st.expander("4. Dynamic Synergy Bundle Evaluation & Logic Check"):
        bundle_audit = []
        for idx, row in edited_bundles.iterrows():
            b_name = row.get("Bundle Name", f"Bundle_{idx}")
            b_disc = row.get("Discount ($)", 0)
            req_raw = row.get("Required Nodes (Comma Separated)", "")
            req_list = [n.strip() for n in str(req_raw).split(",") if n.strip() in nodes]
            
            # Check if all required nodes are selected in the solution
            is_unlocked = all(n in selected_nodes for n in req_list) if req_list else False
            bundle_audit.append({
                "Bundle Name": b_name,
                "Required Nodes": req_raw,
                "Discount Value ($)": b_disc,
                "All Required Nodes Active?": "Yes" if is_unlocked else "No",
                "Bundle Applied in Solution?": "Active" if b_name in active_bundle_names else "Inactive"
            })
        st.dataframe(pd.DataFrame(bundle_audit), use_container_width=True)
