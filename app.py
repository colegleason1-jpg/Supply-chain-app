...
st.markdown(
    "*Enterprise Hybrid Architecture: PuLP Mixed-Integer Core (MES & Bundles) + SciPy/NumPy Monte Carlo Analytics + Live Macro Telemetry.*"
)
st.markdown("------")
...
# ==========================================
# 3.5 LIVE MACRO TELEMETRY AGENT & WIDGET
# ==========================================
@st.cache_data(ttl=1800)
def fetch_realtime_macro_disruptions() -> float:
    """
    Pulls live macro indicators (e.g., benchmark crude/logistics costs) 
    from a public REST endpoint and scales them into a single multiplier.
    Returns 1.0 (neutral) if offline or unreachable.
    """
    try:
        ...
        if response.status_code == 200:
            ...
            if current_price > baseline_anchor:
                macro_multiplier = 1.0 + ((current_price - baseline_anchor) / baseline_anchor) * 0.1
            else:
                macro_multiplier = 1.0
            return round(macro_multiplier, 4)
        else:
            return 1.0
    except Exception:
        return 1.0
...
st.sidebar.markdown("---")
st.sidebar.subheader("🌐 Live Macro Telemetry Feed")
live_macro_multiplier = fetch_realtime_macro_disruptions()

col_t1, col_t2 = st.sidebar.columns(2)
col_t1.metric("Macro Multiplier", f"{live_macro_multiplier}x", delta="Live" if live_macro_multiplier > 1.0 else "Stable")
col_t2.metric("Feed Status", "Active", delta="Secured")
...
# Diminishing Returns Transformation (Macro-scaled Non-linear Marginal Scaling)
...
# ==========================================
bundle_vars = {}                  # Global initialization to prevent NameError scoping issues
...
col1.metric("Effective System Risk", f"{effective_baseline_risk:.2f}%", delta=f"{live_macro_multiplier}x Macro" if live_macro_multiplier != 1.0 else "Normal")
...
with tab3:
    ...
    with st.expander("📌 View Complete System Master Equation (Zero Omissions)", expanded=True):
        st.markdown("### **I. Hybrid Objective Function Selection (Mode Dependent & Macro Fed)**")

        st.markdown("**Target Mode: Total Capital Outlay Minimization Function**")
        st.latex(r"\mathcal{U}_{\text{system}} = \sum_{n \in N} (C_n x_n) - \sum_{b \in B} (D_b \cdot b_v)")

        st.markdown("**Commercial Mode: Portfolio Utility Maximization Function (Live Macro Scaled)**")
        st.latex(r"\mathcal{U}_{\text{system}} = \sum_{n \in N} \left[ w \cdot (\beta_{\text{macro}} \cdot R_n)^{0.85} + (1-w) \cdot L_n \right] x_n")

        st.markdown("---")
        st.markdown("### **II. Structural Operational & Feasibility Constraints (PuLP Mixed-Integer)**")
        st.markdown("1. **Semi-Continuous Scaling & MES Thresholds:**")
        st.latex(r"0.3 \, y_n \le x_n \le 1.0 \, y_n, \quad \forall n \in N, \quad y_n \in \{0, 1\}, \quad x_n \in [0, 1]")

        st.markdown("2. **Capital Budget Cap & Bundle Discounts:**")
        st.latex(r"\sum_{n \in N} (C_n x_n) - \sum_{b \in B} (D_b \cdot b_v) \le B_{\text{total}}")

        st.markdown("3. **Prerequisite Dependency Cascade Capping:**")
        st.latex(r"x_{\text{dependent}} \le x_{\text{prerequisite}}, \quad \forall (\text{dep}, \text{pre}) \in \text{Edges}")

        st.markdown("4. **Synergy Bundle Logic Prerequisites:**")
        st.latex(r"b_v \le y_n, \quad \forall n \in \text{Nodes}(b), \quad b_v \in \{0, 1\}")

        st.markdown("5. **Target Risk Threshold Lower Bound (Macro Adjusted):**")
        st.latex(r"\sum_{n \in N} ((\beta_{\text{macro}} \cdot R_n)^{0.85} x_n) \ge \max\left(0, (\beta_{\text{macro}} \cdot \text{Risk}_{\text{baseline}}) - \text{Risk}_{\text{target}}\right)")

        st.markdown("---")
        st.markdown("### **III. Multivariate Correlated Monte Carlo Simulation Engine (NumPy / SciPy Matrix Core)**")
        st.markdown("1. **User Matrix Definiteness Correction:**")
        st.latex(r"\Sigma_{\text{custom}} = \text{User Matrix} + \epsilon \cdot \mathbf{I} \quad (\text{Eigenvalue-clipped})")

        st.markdown("2. **Cholesky Factorization Matrix:**")
        st.latex(r"\Sigma_{\text{custom}} = L L^T")

        st.markdown("3. **Correlated Normal Perturbation Vector:**")
        st.latex(r"Z_{\text{corr}} = L \cdot Z_{\text{normal}}, \quad Z_{\text{normal}} \sim \mathcal{N}(0, \mathbf{I})")

        st.markdown("4. **Stochastic Outcome Distribution (Live Telemetry Fed):**")
        st.latex(r"\text{Risk}_{\text{sim}}^{(i)} = \max\left(0, (\beta_{\text{macro}} \cdot \text{Risk}_{\text{baseline}}) - \sum_{n \in N} (\beta_{\text{macro}} \cdot R_n)^{0.85} x_n \max\left(0.4, 1.0 + 0.12 \, Z_{\text{corr}, n}^{(i)}\right)\right)")

        st.markdown("5. **Institutional Tail-Risk Value-at-Risk ($P_{90}$):**")
    ...
    with st.expander("2. Runtime Performance Ledger & Parameters"):
        st.markdown(f"""
        * **Active Macro Multiplier:** $\\beta_{{\\text{{macro}}}} = {live_macro_multiplier}$
