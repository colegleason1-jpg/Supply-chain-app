...
if "nodes_df" in st.session_state:
    st.session_state.nodes_df["Node Name"] = st.session_state.nodes_df["Node Name"].astype(str)
    st.session_state.nodes_df["Action"] = st.session_state.nodes_df["Action"].astype(str)
    st.session_state.nodes_df["Cost"] = pd.to_numeric(st.session_state.nodes_df["Cost"], errors='coerce').fillna(0.0).astype(float)
    st.session_state.nodes_df["Risk Reduction (%)"] = pd.to_numeric(st.session_state.nodes_df["Risk Reduction (%)"], errors='coerce').fillna(0.0).astype(float)
    st.session_state.nodes_df["Lead Time Saved (Days)"] = pd.to_numeric(st.session_state.nodes_df["Lead Time Saved (Days)"], errors='coerce').fillna(0.0).astype(float)
    st.session_state.nodes_df["Carbon Impact (Tons)"] = pd.to_numeric(st.session_state.nodes_df["Carbon Impact (Tons)"], errors='coerce').fillna(0.0).astype(float)
