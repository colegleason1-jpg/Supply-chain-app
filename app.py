...
if enable_market_mode:
    ...
    with tab3:
        ...
        with st.expander("📌 View Complete System Master Equation (Zero Omissions)", expanded=False):
            ...
            \\[1em]
            \textbf{IV. Multivariate Correlated Monte Carlo Simulation Engine:} \\[0.3em]
            1. \text{ User Matrix Definiteness Correction:} \quad & \Sigma_{\text{custom}} = \text{User Matrix} + \epsilon \cdot \mathbf{I} \\[0.5em]
            ...
            4. \text{ Stochastic Outcome Distribution:} \quad & \text{Risk}_{\text{sim}}^{(i)} = \max\left(0, (\beta_{\text{macro}} \cdot \text{Risk}_{\text{baseline}}) - \sum_{n \in N} R_n^* x_n \max\left(0.4, 1.0 + 0.12 \, Z_{\text{corr}, n}^{(i)}\right)\right) \\[0.5em]
