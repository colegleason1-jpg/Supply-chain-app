...
if enable_target_mode:
    ...
    for n in nodes:
        prob += x[n] >= 0.3 * y[n]  # Minimum Efficient Scale lower bound
        prob += x[n] <= 1.0 * y[n]  # Upper activation cap
...
# --- ADVANCED CHOLESKY MONTE CARLO ENGINE (NUMPY/SCIPY BACKED) ---
def run_hybrid_monte_carlo(base_risk, scales_dict, marginal_dict, corr_df_matrix, nodes_list, iterations):
    ...
    # Eigenvalue clipping for positive semi-definiteness guarantee via SciPy/NumPy linear algebra
    try:
...
with tab2:
    st.subheader("Enterprise Budget Sensitivity Analysis (Hybrid Engine)")
    sweep_results = []
...
with tab3:
    ...
    with st.expander("2. Runtime Performance Ledger & Parameters"):
        ...
        * **Institutional Tail-Tail Risk ($P_{90}$ VaR, $N={num_iterations:,}$):** $\mathbf{{{mc_results['P90_Risk']:.2f}\\%}}$
        """)
