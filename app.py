    ...
    if force_refresh or elapsed >= 1800 or st.session_state.last_market_sync_time == 0.0:
        ...
        for idx, row in local_df.iterrows():
            ...
            try:
                ...
                # --- FIXED: BUG 1 (yfinance Safe Extraction Wrapper) ---
...
def generate_market_pdf_report(market_df: pd.DataFrame) -> bytes:
    ...
    # --- FIXED: BUG 2 (ReportLab Text Clipping Leading Property) ---
...
# Dual-Layer Amplified Marginal Utility Function
...
if enable_market_mode:
    ...
    with tab2:
        ...
        for b in budget_range:
            ...
            # --- FIXED: BUG 3 (Matrix Scope Leak inside Budget Sweep Loop uses isolated sub_x) ---
    ...
    with tab3:
        ...
        with st.expander("📌 View Complete System Master Equation (Zero Omissions)", expanded=False):
            ...
            1. \text{ Live Global Macro Multiplier Factor ($\beta_{\text{macro}}$):} \quad & \beta_{\text{macro}} = \begin{cases} 1.0 + \left(\frac{P_{\text{brent}} - P_{\text{anchor}}}{P_{\text{anchor}}}\right) \cdot 0.15 & \text{if } P_{\text{brent}} > P_{\text{anchor}} \\ 1.0 & \text{otherwise} \end{cases} \\[0.5em]
            2. \text{ Macro-Scaled Diminishing Returns Utility ($R_n^*$):} \quad & R_n^* = \left(\beta_{\text{macro}} \cdot R_n\right)^{0.85}, \quad \forall n \in N_{\text{active}}
            \\[1em]
    ...
    with st.sidebar:
        ...
        if raw_user_query:
            ...
            # --- FIXED: BUG 1 (Removed infinite re-run loop 'else: st.rerun()') ---
            if action_taken:
                time.sleep(0.4)
                st.rerun()
