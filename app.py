    ...
    try:
        ...
        if df_uploaded is not None:
            ...
            if not all(col in df_uploaded.columns for col in required_cols):
                ...
                normalized_df["Action"] = df_uploaded.iloc[:, 1].astype(str) if len(df_uploaded.columns) > 1 else ["Imported entry action"] * nrows
                normalized_df["Cost"] = pd.to_numeric(df_uploaded.iloc[:, 2], errors='coerce').fillna(100000) if len(df_uploaded.columns) > 2 else [100000.0] * nrows
